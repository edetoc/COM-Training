...
Modules 1–4 built everything by hand so you'd understand the machinery. Nobody writes production COM that way — and in fact you already stopped: the server behind every Lab 5.1 and 5.2 client was generated by the **ATL** wizard, and you used its `IDispatchImpl`, its connection point container and its `.rgs` registration without being told what any of them were doing.

This module is where that debt comes due. It covers the frameworks that remove the boilerplate — starting with the one you have already been running — and, more importantly, what each is doing underneath so you can debug it.
...
- [6.7 LAB 6.1 — The hand-written server vs the ATL one](#67-lab-61--the-hand-written-server-vs-the-atl-one)
...
Count what one COM object needs before any framework gets involved:
...
You have been on both sides of that table. The first five rows you wrote yourself, in Module 2's in-proc server. The last three you never wrote at all — `IDispatchImpl` answered every `Invoke` in Lab 5.1, and `CProxy_ICalculatorEvents` delivered every `OnProgress` in Lab 5.2. The only part of the event machinery you did write by hand was the **sink**, which is the client's side and the one part ATL cannot do for you.
...
## 6.7 LAB 6.1 — The hand-written server vs the ATL one
...
> - **Time:** ~1 h, since nothing needs building.

**You have already built the "after".** The ATL server from Lab 5.1's starting point *is* this lab's second half — same object, same methods, same `Precision` property, plus the events. Do not build it again.

**The server was never the deliverable; the comparison is.** Counting what disappeared, and then stepping into ATL to see where it went, is what stops ATL being a black box the first time it misbehaves.

So open both at once — [`labs/stage-2-inproc-server/`](../labs/stage-2-inproc-server/) and your `TrainingCalc` project.

> If you skipped Module 5, work through [`labs/stage-4-atl-server/README.md`](../labs/stage-4-atl-server/README.md) now (15–20 minutes) and come back.
...
Take the ATL server you have been using since Lab 5.1 and consume it three ways:
