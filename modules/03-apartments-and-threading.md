...
Apartments — the STA, the MTA, and the neutral apartment — what `ThreadingModel` actually decides, and how to find out which apartment a thread really joined. Then the two things that break: passing a raw interface pointer across a boundary, with the four safe ways to move one and the registered marshaling support they all depend on, and reentrancy, where an STA runs other people's code in the middle of your method. It closes with `IMessageFilter` and four deadlock patterns, each shown as it appears in a debugger.
...
- [3.12 .NET and apartments](#312-net-and-apartments)
- [3.13 Checkpoint](#313-checkpoint)
- [3.14 Rules to carry forward](#314-rules-to-carry-forward)
...
> **Use it constantly.** It answers in one call the question §3.3's whole table is about — *which apartment am I actually in?* — and it is the fastest way to check a customer's **claim** about which apartment their code runs on against reality. Claims and reality differ more often than you'd expect.
...
Before going further, one thing has been glossed over: **COM cannot invent a proxy.** To build one it must know the exact shape of every method on the interface — how many there are, what each parameter is, which are `[in]` and which are `[out]`, how long the arrays are. None of that is in the vtable; a vtable is just function pointers.

That description has to be supplied, and it lives outside your component in one of two forms:

| Form | What it is | Registered under |
|---|---|---|
| **A proxy/stub DLL** | A DLL generated by MIDL from your IDL, containing the proxy and stub code for each interface | `HKCR\Interface\{IID}\ProxyStubClsid32` |
| **A type library** | The interface described in a `.tlb`, marshaled by the system's Automation marshaler | `HKCR\Interface\{IID}\TypeLib` |

So marshaling is a **per-interface, registered capability**, not something every interface has automatically. When you ask COM to move a pointer across an apartment boundary, it looks up that IID and, finding nothing, refuses:

> `0x80040155` **`REGDB_E_IIDNOTREG`** — *"Interface not registered."* Read it as: **this interface has no marshaling support installed**, so no proxy can be built. It is not about the *class* being unregistered — that is `REGDB_E_CLASSNOTREG` (`0x80040154`), a different error with a different fix.

This is the single most common reason a cross-apartment call fails on a correctly written client, and it looks nothing like a threading bug. Two consequences worth carrying:

- **An interface that works perfectly in-process may be unusable across a boundary.** In-apartment calls are direct vtable calls and need no description at all (§3.2), which is why the gap stays hidden until the first thread or process boundary is crossed.
- **Many system interfaces already have it.** `IStream`, `IDataObject` and friends ship with marshaling support, which is why they cross apartments with no work from you. Your own interfaces start with none — Lab 3.1 shows both cases side by side, and **Module 4** is where you fix it for your own.
...
§3.5 said a raw interface pointer must never cross an apartment boundary. This lab shows what that looks like in practice, as four experiments the program runs in order.

**Experiment 1 — the WRONG way.** Hand a raw pointer to another thread and call through it. **The call succeeds**, because calling through a raw pointer is an ordinary C++ virtual call and no COM code runs that could stop you. The object ends up executing on the wrong thread without the protection it was promised, and nothing reports an error. This is the experiment that should worry you.

**Experiments 2 and 3 — the RIGHT way.** Marshal the pointer properly: once through a stream, then repeatedly through the Global Interface Table. Now you see the **proxy** — a second pointer, at a different address, standing in for the object.

**Experiment 4 — the right way, on your own interface.** It is refused, and the error tells you why `ICalculator` cannot cross an apartment boundary until Module 4.

So the lab's uncomfortable result is that the wrong way appears to work while the right way, applied to your own component, does not work *yet*.

**Why the code uses two objects: your `Calculator` and a stream.** Marshaling only works on an interface COM already has a description of, and `ICalculator` has none until you write its IDL in Module 4. So experiments 2 and 3 borrow an object that already has one: a **stream**, created by `CreateStreamOnHGlobal`. It is an ordinary COM object, its marshaling support ships with Windows, and it needs no registration — which makes it a perfect stand-in for showing what a proxy looks like.

| Experiment | Object used | Why |
|---|---|---|
| 1 — raw pointer (**wrong**) | `Calculator` | Nothing is marshaled, so no description is needed |
| 2 — stream marshaling (**right**) | a stream | Needs an interface COM can marshal |
| 3 — the GIT (**right**) | a stream | Same |
| 4 — marshal your own interface (**right, but refused**) | `Calculator` | The point is to see COM turn it down |

**Setting up the project.** Add a new C++ console project to the Stage 2 solution, then add the existing **`Calculator.h`** from `labs/stage-2-inproc-server/` to it (Project → Add Existing Item), so `ICalculator`, `IID_ICalculator` and `CLSID_Calculator` are declared. Without it, `ICalculator` will not resolve. No other project settings are needed — the defaults are fine, and `ole32.lib` is already linked.

The include order below matters, and not in the way you might guess. `<initguid.h>` must come **after** `<atlbase.h>`, not first: it switches `DEFINE_GUID` from *declaring* a GUID to *defining* one, and if it is in effect while the system and ATL headers are parsed you get a wall of `error C2065: 'GUID_NULL': undeclared identifier`. Put it immediately before your own header so it applies to that header only.
...
#include <initguid.h>     // must come AFTER the system headers, immediately before Calculator.h
#include "Calculator.h"   // ICalculator, IID_ICalculator, CLSID_Calculator

// ============ EXPERIMENT 1: the wrong way - a raw pointer, no marshaling ====
...
DWORD WINAPI WorkerRaw(LPVOID pv)
{
    ...
    printf("[raw]  hr=0x%08X r=%ld   <-- nothing stopped us. That is the danger.\n", hr, r);
    CoUninitialize();
    return 0;
}

// ============ EXPERIMENT 2: marshal once, through a stream =================
DWORD WINAPI WorkerStream(LPVOID pv)
{
    CoInitializeEx(nullptr, COINIT_MULTITHREADED);
    IStream* pMarshalStream = static_cast<IStream*>(pv);

    CComPtr<IStream> spProxy;
    HRESULT hr = CoGetInterfaceAndReleaseStream(pMarshalStream, IID_IStream,
                                                reinterpret_cast<void**>(&spProxy));
    if (SUCCEEDED(hr))
    {
        ULONG written = 0;
        hr = spProxy->Write("hello", 5, &written);
        printf("[strm] proxy=%p  Write hr=0x%08X wrote=%lu\n", spProxy.p, hr, written);
    }
    else printf("[strm] unmarshal failed 0x%08X\n", hr);   // never fail silently
    spProxy.Release();          // release BEFORE CoUninitialize - see the note below
    CoUninitialize();
    return 0;
}

// ============ EXPERIMENT 3: marshal repeatedly, through the GIT ============
...
DWORD WINAPI WorkerGit(LPVOID pv)
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        CComPtr<IStream> spLocal;
        HRESULT hr = a->pGit->GetInterfaceFromGlobal(a->cookie, IID_IStream,
                                                     reinterpret_cast<void**>(&spLocal));
        ULONG written = 0;
        if (SUCCEEDED(hr)) hr = spLocal->Write("x", 1, &written);
        printf("[git]  iter %d hr=0x%08X proxy=%p\n", i, hr, spLocal.p);
    }
    CoUninitialize();
    return 0;
}
...
static void Run()
{
    ...
    HRESULT hr = CoCreateInstance(CLSID_Calculator, nullptr, CLSCTX_INPROC_SERVER,
                                  IID_ICalculator, reinterpret_cast<void**>(&spCalc));
    if (FAILED(hr)) { printf("create failed 0x%08X\n", hr); return; }
    printf("Calculator in main STA = %p\n", spCalc.p);

    RawArgs raw{ spCalc.p };
    HANDLE h = CreateThread(nullptr, 0, WorkerRaw, &raw, 0, nullptr);
    WaitForSingleObject(h, INFINITE); CloseHandle(h);

    // ---- EXPERIMENTS 2 and 3 both use this stream ---------------------
    // Windows already ships marshaling support for IStream, so no registration.
    CComPtr<IStream> spStream;
    CreateStreamOnHGlobal(nullptr, TRUE, &spStream);
    printf("\nStream object in main STA = %p\n", spStream.p);

    IStream* pMarshal = nullptr;
    hr = CoMarshalInterThreadInterfaceInStream(IID_IStream, spStream, &pMarshal);
    if (SUCCEEDED(hr))
    {
        h = CreateThread(nullptr, 0, WorkerStream, pMarshal, 0, nullptr);
        // Waits, but keeps pumping - so this STA can still serve the proxy's calls.
        DWORD idx;
        CoWaitForMultipleHandles(COWAIT_DEFAULT, INFINITE, 1, &h, &idx);
        CloseHandle(h);
    }
    else printf("[strm] marshal failed 0x%08X\n", hr);

    // ---- EXPERIMENT 3 -------------------------------------------------
    CComPtr<IGlobalInterfaceTable> spGit;
    spGit.CoCreateInstance(CLSID_StdGlobalInterfaceTable, nullptr, CLSCTX_INPROC_SERVER);
    DWORD cookie = 0;
    spGit->RegisterInterfaceInGlobal(spStream, IID_IStream, &cookie);
    ...
    STATSTG st{};
    spStream->Stat(&st, STATFLAG_NONAME);
    printf("\nbytes written by the worker threads = %llu\n", st.cbSize.QuadPart);

    // ---- EXPERIMENT 4: try to marshal YOUR interface ------------------
    IStream* pCalcMarshal = nullptr;
    hr = CoMarshalInterThreadInterfaceInStream(IID_ICalculator, spCalc, &pCalcMarshal);
    printf("\nmarshaling ICalculator: hr=0x%08X\n", hr);
}
...
int main()
{
    ...
    Run();   // scoped so every smart pointer is released before CoUninitialize
...
> **`CoWaitForMultipleHandles` is not an ordinary wait.** It blocks your code until the thread handle is signalled, but on an STA it **supplies a message loop of its own** while it waits: it waits on the handle *and* the message queue together, dispatching whatever arrives before going back to waiting. Since a cross-apartment call reaches an STA as a message to its hidden window (§3.2), that loop is what keeps this thread able to serve the proxy. Use `WaitForSingleObject` here and the thread stops dispatching entirely — see exercise 2. (On an MTA there is no pump, so it behaves like a plain wait.)

### Run it and read the output

```
main thread 38948 is an STA

Calculator in main STA = 0000020C2411C720
[raw]  hr=0x00000000 r=3   <-- nothing stopped us. That is the danger.

Stream object in main STA = 0000020C2412A1B0
[strm] proxy=0000020C2410AE28  Write hr=0x00000000 wrote=5
[git]  iter 0 hr=0x00000000 proxy=0000020C2413EA48
[git]  iter 1 hr=0x00000000 proxy=0000020C2413EA48
[git]  iter 2 hr=0x00000000 proxy=0000020C24123928

bytes written by the worker threads = 8

marshaling ICalculator: hr=0x80040155
```

Your addresses will differ — they are heap allocations that change on every run. What matters is how they relate to each other.

The program runs four experiments in the order they appear in `Run()`. Here is each one, with the lines it printed.

#### Experiment 1 — the raw pointer (the wrong way)
...
`S_OK`, and the right answer. COM did not stop you, did not warn you, and will not stop you in production either — because no COM code ran. `a->p->Add(...)` loaded an address from a vtable and jumped to it, exactly like any C++ virtual call. `Calculator::Add` executed **on the worker thread**, inside an object registered `ThreadingModel = Apartment` that therefore has no locks of its own.

**This is not luck.** There is no check in that call path to pass or fail, so it will succeed every time you run it. What is left to chance is the *damage*: corrupting the object needs two threads inside it at once, and here a single call runs with nothing to collide with. Add concurrency and the same code produces torn fields, a mangled reference count, or a crash somewhere that looks unrelated.

That is the shape of the bug you will be handed in support: reliably fine in testing, occasionally catastrophic in production, with a stack trace pointing nowhere near this line.

#### Experiment 2 — marshal once, through a stream (the right way)
...
Two different addresses. `...2412A1B0` is the real stream, living in the main STA; `...2410AE28` is what the worker thread received. Same logical object, two pointers — this is the proxy, and it is the thing §3.2's round-trip diagram is about. The worker holds something that looks exactly like an `IStream`, and every call it makes is packaged up and delivered to the STA.

Note also that `Write` returned `S_OK` and reported 5 bytes. The proxy is not a polite fiction; the call arrived.

#### Experiment 3 — marshal repeatedly, through the GIT (the right way)
...
Three successful calls, each through a proxy the worker obtained from the GIT by **cookie**, not by pointer.

Look at iterations 0 and 1: the *same* address, while iteration 2 differs. Nothing is being cached. `GetInterfaceFromGlobal` builds a fresh proxy every call, the `CComPtr` releases it at the end of each loop body, and the allocator is then free to hand the identical block back for the next one. Whether that happens is luck, and it varies between runs.

There is a lesson in that beyond the GIT: **an interface pointer identifies an object only while you hold a reference to it.** Once released, the address can be recycled and means nothing. That is also why the GIT gives you a cookie — it stays valid, the pointer does not.
...
Read back by the **main thread**, from the real object: 5 bytes of `hello` from experiment 2, plus three single `x` from experiment 3. Every call made through every proxy reached the object in the STA. That is location transparency working.

#### Experiment 4 — the right way, on your own interface

```
marshaling ICalculator: hr=0x80040155
```

`REGDB_E_IIDNOTREG`. Experiments 2 and 3 worked because Windows ships marshaling support for `IStream`. Ask for the same treatment for `ICalculator` and COM refuses: it has no description of your interface and cannot invent one (§3.5).

So your object is stuck in its apartment — the only way to use it from another thread is the illegal one from experiment 1.

That single line is the reason Module 4 exists. Writing the IDL, running MIDL and registering the proxy/stub is what turns this `0x80040155` into `S_OK`, and **[Lab 4.2](04-idl-and-marshaling.md#47-lab-42--break-and-fix-marshaling)** has you come back and watch it flip.

### Exercises

1. **Look at the two vtables in WinDbg.** Break after the worker has its proxy and run `dps` on both pointers. The STA one resolves to the stream implementation in `combase`/`ole32`; the MTA one resolves to proxy machinery. Same interface, two entirely different sets of function pointers — location transparency, at the byte level.
2. **Replace `CoWaitForMultipleHandles` with `WaitForSingleObject`** in the stream case. It hangs, and the reason is worth understanding. Both calls block your code until the thread handle is signalled — the difference is what the *thread* does meanwhile. `WaitForSingleObject` blocks it completely. `CoWaitForMultipleHandles`, on an STA, runs a modal loop that keeps dispatching incoming COM calls while it waits, so the apartment can still serve the proxy. Take that away and you get Pattern 1 from §3.8: the worker waits forever for a reply that the main thread is no longer able to deliver, and the main thread waits forever for the worker. (On an MTA there is no pump and no difference — it is an ordinary wait.)
3. **Delete `RevokeInterfaceFromGlobal`.** Add ref-count tracing and watch the stream never reach zero. The GIT holds a reference until you revoke the cookie, and forgetting it is the classic GIT leak — the whole reason `IAgileReference` (§3.5, Option C) exists.
4. **Call the raw `ICalculator` pointer from the worker in a loop, thousands of times, while the main thread also calls it.** The `[raw]` route will keep appearing to work. Write in your journal what that means for a bug report that says "we've been doing this for years and it's fine."
5. **Swap `CreateStreamOnHGlobal` for your own object** and confirm you get `0x80040155`. That is the experiment Lab 4.2 completes.
...
> - **Depends on:** nothing. The listing below is complete and needs no registered component — it uses a stream, exactly as Lab 3.1 did.
> - **Starting point:** a new C++ console project. Copy the listing in whole.
> - **Time:** ~2 h.

Lab 3.1 ended with a warning: `CoWaitForMultipleHandles` keeps an STA pumping, and `WaitForSingleObject` does not. This lab makes a program that ignores that warning, so you can watch it hang and then take it apart in a debugger. It is the single best preparation for real hang tickets.

**The deadlock in one sentence:** the main thread waits for the worker without pumping, while the worker waits for the main thread to serve a cross-apartment call. Neither can move.

Copy this whole file. It hangs on purpose.
...
{
    CoInitializeEx(nullptr, COINIT_MULTITHREADED);   // this thread joins the MTA
    {
        printf("[worker] calling GetInterfaceFromGlobal\n"); fflush(stdout);

        CComPtr<IStream> sp;
        HRESULT hr = g_pGit->GetInterfaceFromGlobal(g_cookie, IID_IStream,
                                                    reinterpret_cast<void**>(&sp));
        printf("[worker] got proxy hr=0x%08X\n", hr); fflush(stdout);

        ULONG written = 0;
        hr = sp->Write("x", 1, &written);
        printf("[worker] Write returned 0x%08X\n", hr); fflush(stdout);
    }
...
int main()
{
    ...
    CComPtr<IGlobalInterfaceTable> spGit;
    spGit.CoCreateInstance(CLSID_StdGlobalInterfaceTable, nullptr, CLSCTX_INPROC_SERVER);
    spGit->RegisterInterfaceInGlobal(spStream, IID_IStream, &g_cookie);
    g_pGit = spGit;

    HANDLE h = CreateThread(nullptr, 0, Worker, nullptr, 0, nullptr);

    printf("[main] waiting without pumping...\n"); fflush(stdout);
    WaitForSingleObject(g_hWorkerDone, INFINITE);   // <-- THE BUG. Hangs here forever.
    printf("[main] never reached\n");

    CloseHandle(h);
    spGit->RevokeInterfaceFromGlobal(g_cookie);
    CoUninitialize();
    return 0;
}
```

### Run it: what a hang looks like from outside

```
[main] waiting without pumping...
[worker] calling GetInterfaceFromGlobal
```

Then nothing, forever. Kill it with Ctrl+C.

Note **where** it stops. The worker never printed `got proxy`, so it is not stuck on the `Write` call — it never got that far. Obtaining the proxy is itself a cross-apartment operation: unmarshaling has to reach into the STA that owns the stream, and that STA is already blocked in `WaitForSingleObject`. The hang happens at the *first* thing that needs the STA, whatever that happens to be.

Also note the CPU is idle. Two threads are waiting on each other; nothing is spinning. **A COM deadlock costs no CPU**, which is why "the process is using 0% and doing nothing" is the classic report.

### The one line that causes it, and three ways to fix it

The bug is this line in `main`:

```cpp
WaitForSingleObject(g_hWorkerDone, INFINITE);   // blocks the thread AND the pump
```

After you have finished the WinDbg work below, try each of these and confirm the program runs to completion:

| Fix | Change | Why it works |
|---|---|---|
| **A** | Replace that line with `DWORD idx; CoWaitForMultipleHandles(COWAIT_DEFAULT, INFINITE, 1, &g_hWorkerDone, &idx);` | Waits *and* keeps dispatching, so the STA can still serve the worker's call |
| **B** | Change `main`'s `CoInitializeEx` to `COINIT_MULTITHREADED` | With both threads in the MTA there is no apartment boundary and no pump to starve — the call is served on an RPC thread |
| **C** | Do not block the STA at all | Restructure so the worker posts a message or signals completion asynchronously. This is what real UI code must do |

With fix A the output completes:

```
[main] waiting without pumping...
[worker] calling GetInterfaceFromGlobal
[worker] got proxy hr=0x00000000
[worker] Write returned 0x00000000
[main] never reached
```
...
2. Open in WinDbg, load symbols, then:
   ...
   - **Worker thread:** `ntdll!NtWaitForSingleObject` → `combase!CSyncClientCall::SwitchAptAndDispatchCall` → `combase!CRpcChannelBuffer::SendReceive` → `RPCRT4!NdrpClientCall` → the GIT call it is stuck in.
3. Confirm the shape:
   ```
...
5. Now apply fixes **A**, **B** and **C** from the table above, one at a time, and re-run after each. Confirm the program reaches `[main] never reached` every time — and that `~*kb` no longer shows a thread parked in `SendReceive`.
